function cleanLC_DualArea_FIRA(clean_name)
...
% Cleans up and saves structure siteData (in file "clean_name"):
%
% "siteData" contains the following:
%
...
% Make filter for running linear slope (for pupil):
FILTER_HW = 75;
...
ai_p = find(strncmp('Pup', FIRA.analog.name, 3) | strncmp('pup', FIRA.analog.name, 3));
ai_h = find(strncmp('HEy', FIRA.analog.name, 3) | strncmp('hor', FIRA.analog.name, 3));
ai_v = find(strncmp('VEy', FIRA.analog.name, 3) | strncmp('ver', FIRA.analog.name, 3));
...
ai_l = find(strncmp('AD', FIRA.analog.name, 2) | strncmp('unk', FIRA.analog.name, 3) | strncmp('LFP', FIRA.analog.name, 3));

% Trial selection stuff:
...
for tt = 1:num_trials
    ...
    if fp_end > fp_start + 100 && fp_start > 0
        ...
        %% SPECIAL CASE for microstim (as per Josh):
        ...
        for ss = 1:nsacs-1
            ...
            if abs(medx) < 5 && abs(medy) < 5
                ...
                offset = find(abs(FIRA.analog.data(tt,ai_h).values(i1:i2))<fix_accept & ...
                    abs(FIRA.analog.data(tt,ai_v).values(i1:i2))<fix_accept, 1);
                i1 = i1 - 1 + offset;
                if i1 + 200 < i2 % at least 200 ms
                    
                    % check good fixation
                    Lfix = abs(FIRA.analog.data(tt,ai_h).values(i1:i2))<fix_accept & ...
                        abs(FIRA.analog.data(tt,ai_v).values(i1:i2))<fix_accept & ...
        ...
        if gi(1) > 200
            ...
            if ~isempty(ai_hr)
                for ss = 1:length(ai_hr)
                    FIRA.analog.data(tt,ai_hr(ss)).start_time = gi(2);
                end
            end
            
            if ~isempty(ai_ee)
                for ss = 1:length(ai_ee)
                    FIRA.analog.data(tt,ai_ee(ss)).start_time = gi(2);
                end
            end
...
% clean_name = 'Sp_030917_13_200_5_900_FixnBeepStim-01_CLEAN';
% save it
