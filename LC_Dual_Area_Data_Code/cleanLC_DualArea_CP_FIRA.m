function cleanLC_DualArea_CP_FIRA(clean_name)

% 062116 - Sidd
% 063016 - Modified from cleanLC_DualArea_FIRA (that was for Fixation task only).
% For task 740 ... 2AFC reward based change-point task.
%
% Create "clean" data structures for dual-area recording data.
% Modified from "cleanLCP_FIRA" to account for additional analog signals:
% (a) LFP's from each recording site on V-Probe and from single LC electrode,
% (b) eeg signals,
% (c) pulse-ox signal.

% function cleanLC_DualArea_FIRA(clean_name, pulseOxFlag)
...
%   10 ... choice - which TG did mnky choose (0,1,2 for none, TG1, TG2).
%   11 ... tg1_on - this a nan if TG2 was chosen for reward; this is a time value - not recoded wrt fix start
%   12 ... tg2_on - this a nan if TG1 was chosen for reward; time is a time value - not recoded wrt fix start
%   13 ... rew_or_not - was this trial rewarded? 1:rewarded, 0: not rewarded.
%   14 ... trialType - 1 for regular reward, 2 for BEEP + boost reward
%   15 ... trialid - 300, 302, 304 for 80:20, 20:80 or 40:40 respectively
...
% create cell array
siteData  = cell(1,9); % ecodes, eye, spikes, LFP, pupil events, microsaccades

% Analog channel indices
% These are stable from 2015 bs mapping onwards:

% Eye-position and pupil:
ai_p = find(strncmp('Pup', FIRA.analog.name, 3));
ai_h = find(strncmp('HEy', FIRA.analog.name, 3));
ai_v = find(strncmp('VEy', FIRA.analog.name, 3));

if isempty(ai_p) || isempty(ai_h) || isempty(ai_v)
    disp('BAD ANALOG CHANNELS')
end

% Heart rate is from pulse-ox:
ai_hr = find(strncmp('Pul', FIRA.analog.name, 3));

% EEG channels - there are four of these:
ai_ee = find(strncmp('Sl', FIRA.analog.name, 2));

% Now we have 9 LFP channels.... get them all:
ai_l = find(strncmp('AD', FIRA.analog.name, 2));
...
tr_id = getFIRA_ec([], 'trialid'); % Added for CP task.
...
for tt = 1:num_trials
    ...
    if fp_end > fp_start + 100 && fp_start > 0
        ...
        if gi(1) > 200
            ...
            % Re-code analog start time:
            if ~isempty(ai_l) || ~isempty(ai_hr) || ~isempty(ai_ee)
                for ss = 1:length(ai_l)
                    FIRA.analog.data(tt,ai_l(ss)).start_time = gi(2);
                end
            end
...
% ecodes (Sidd - similar to old fixation data structure - with 10-17 added for the choice task):
%   1 ... fix start time wrt fixation on
%   2 ... fix end time wrt fix start time (fix duration)
%   3 ... reported correcdt
...
%   9 ... ELESTM on time (when appropriate), wrt fix start time
%   10 ... choice - which TG did mnky choose (0,1,2 for none, TG1, TG2).
%   11 ... tg1_on - this is a nan if TG2 was chosen for reward; this is a time value - not recoded wrt fix start
%   12 ... tg2_on - this is a nan if TG1 was chosen for reward; time is a time value - not recoded wrt fix start
%   13 ... rew_or_not - was this trial rewarded? 1:rewarded, 0: not rewarded.
%   14 ... trialType - 1 for regular reward, 2 for BEEP + boost reward.
%   15 ... trialid - 300, 302, 304 for 80:20, 20:80 or 40:40 respectively.
%   16 ... sac1_time, wrt to fix start time.
%   17 ... rew1_time - regular (not boost) reward time, wrt to fix start time.
...
siteData{1} = [ ...
    ...
    getFIRA_ec(Lgood, 'choice'), ...
    getFIRA_ec(Lgood, 'tg1_on'), ...
    getFIRA_ec(Lgood, 'tg2_on'), ...
    getFIRA_ec(Lgood, 'rew_or_not'), ...
    getFIRA_ec(Lgood, 'trialType'), ...
    getFIRA_ec(Lgood, 'trialid'), ...
    getFIRA_ec(Lgood, 'sac1_time') - fix_times(Lgood,1), ...
    getFIRA_ec(Lgood, 'rew1_time') - fix_times(Lgood,1), ...
    getFIRA_ec(Lgood, 'fp_off') - fix_times(Lgood,1)];
...
if ~isempty(ai_l)
    for ll = 1:length(ai_l)
        siteData{4} = cat(2, siteData{4}, ...
            {FIRA.analog.data(Lgood,ai_l(ll)).values}');
    end
end

% EEG:
if ~isempty(ai_ee)
    for ll = 1:length(ai_ee)
        siteData{8} = cat(2, siteData{8}, ...
            {FIRA.analog.data(Lgood,ai_ee(ll)).values}');
    end
end

% Pulse-Ox:
if ~isempty(ai_hr)
    for ll = 1:length(ai_hr)
        siteData{9} = cat(2, siteData{9}, ...
            {FIRA.analog.data(Lgood,ai_hr(ll)).values}');
    end
end
...
siteData{7} = {FIRA.spikes.id FIRA.analog.name};
