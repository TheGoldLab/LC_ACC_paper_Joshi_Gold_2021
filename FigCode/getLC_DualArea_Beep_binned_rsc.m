function [beep_rsc_] = getLC_DualArea_Beep_binned_rsc(siteData,numTrialsLim,fi)
...
% Create binsizes.
tt_time = 1000;
% bs = linspace(100,tt_time,10); % Before 042921.
bs = linspace(200,tt_time,5); % 042921.
...
%% Get other important things.

sps_LC = siteData{3}(LBeep,LC_ch); % LC spikes.
sps_ACC = siteData{3}(LBeep,ACC_ch); % ACC spikes.

% Want 1 sec of ACC spikes after beep.
% gti = find(beepTime>0 & fd-beepTime>=tt_time+100);
% gti = find(beepTime>1000 & fd-beepTime>=tt_time+100); % 050521.
gti = find(beepTime>1000 & fd-beepTime>=tt_time+1); % 050521.
ngt = length(gti);
...
for ai = 1:n_ACC
    ...
    for bsi = 1:nbs % Iterate over binsizes.
        ...
        for ti = 1:ngt
            ...
            % ts_fix = ts(ts>-tt_time & ts<= tt_time); % Get spikes from the 1sec after beep.
            ...
            ts_fixA_early = ts(ts>0 & ts<=500);
            ts_fixA_late = ts(ts>500 & ts<=1000);
            ntsB(ti) = length(ts_fixB);
            ntsA(ti) = length(ts_fixA);
            ntsA_early(ti) = length(ts_fixA_early);
            ntsA_late(ti) = length(ts_fixA_late);
            
            %             nbef(ti) = length(ts_fixB);
            %             naft(ti) = length(ts_fixA);
        ...
        nA_binnedA_E{ai} = ntsA_early;
        nA_binnedA_L{ai} = ntsA_late;
...
for nu = 1:n_LC
    ...
    phasicYesNo2 = nans(1,length(gti));
    phasicYesNo3 = nans(1,length(gti));
    phasicYesNo4 = nans(1,length(gti));
...
if ~isempty(phasicU)
    for li = 1:n_LC
        ...
        if ~isempty(phasicU{li})
            ...
            out_rscA_PE = nans(n_ap,5);
            out_rscA_PL = nans(n_ap,5);
            if length(hi_sp_ind)>=numTrialsLim
                for nbi = 1:bsi % Iterate over number of binsizes used (9) - drop the 1msec bin size!
                    ...
                    for ai = 1:n_ap
                        ...
                        if (nA_rate(a1) >= 1 & nA_rate(a2) >= 1) % 042821
                            ...
                            if nbi == 1
                                ...
                                apcount1 = nA_binnedA_E{a1}(hi_sp_ind); apcount2 = nA_binnedA_E{a2}(hi_sp_ind);
                                rsc_result = get_pwc(apcount1(:),apcount2(:),numTrialsLim);
                                out_rscA_PE(ai,:) = [rsc_result.RSC rsc_result.pval fi li ai];
                                
                                apcount1 = nA_binnedA_L{a1}(hi_sp_ind); apcount2 = nA_binnedA_L{a2}(hi_sp_ind);
                                rsc_result = get_pwc(apcount1(:),apcount2(:),numTrialsLim);
                                out_rscA_PL(ai,:) = [rsc_result.RSC rsc_result.pval fi li ai];
                                
                            end
                            
                            % Phasic response Beep trials.
                    ...
                    out_rsc_binned{nbi} = {out_rscB_P out_rscA_P out_rscB_All out_rscA_All mscB vscB mscA vscA out_rscA_PE out_rscA_PL};
                end % Bins.
...
%% NOW - calculate ACC pairwise spike-count correlations. Case: NO Phasic response.
