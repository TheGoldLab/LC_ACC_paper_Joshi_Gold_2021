function [LC_sp_] = getLC_Fano(siteData,mmnum,minNumTrials)
%
...
if mmnum >=3
    ...
    LC_ch = LC_ch(1:end-1); % 2016 LC pupil data set was sorted with MU as the last channel - so drop this for now for Ci and Oz old data!!!
end
...
nL_count = cell(1,n_LC);
nL_count1 = cell(1,n_LC);
nL_count2 = cell(1,n_LC);
nL_count3 = cell(1,n_LC);

% Get LC trial spikes..
sps_LC = siteData{3}(LnoBeep,LC_ch); % LC spikes.
sps_LC_beep = siteData{3}(LBeep,LC_ch); % LC spikes.

% Trial indices for fixation duration > 3sec.
gti = find(fd>=2000);
% gti_beep = find(beepTime>=2000 & fd_beep-beepTime>=1100); % 051821.
...
FF_unit = nans(n_LC,nb2);
% FF_unit = nans(n_LC,2001);
nL = cell(1,n_LC);
nL1 = cell(1,n_LC);
for ai = 1:n_LC
    ats = sps_LC_beep(gti_beep,ai);
    ...
    for ti = 1:length(gti_beep)
        ...
        ts_fix_aft_phasic = ts(ts>=200 & ts <1000); % Get spikes after beep period, but from after phasic response.
    ...
    nL{ai} = bs; % Binned spikes for this unit - ie - binary spike trains.
    nL1{ai} = bs_1; % Binned spikes for this unit - ie - binary spike trains.
    
    % pause
    
    numsp = sum(bs,2);
    g_ind = find(numsp>0);  % Used >1 prior to 121119.
    bs = bs(g_ind,:);
    if length(g_ind)>=minNumTrials
        ...
        %         % Smooth.
        %         unit_FF_Sm = smooth(unit_FF,3); unit_FF_Sm = unit_FF_Sm'; % 092420
        %         FF_unit(ai,:) = unit_FF_Sm;
...
LC_sp_ = {nL_count nL_count1 nL_count2 nL_count3 nL FF_unit nL1};
