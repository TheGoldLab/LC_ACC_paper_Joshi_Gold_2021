...
if nargin >= 8 && ~isempty(al)
    ...
    if centerTitle == 0
        if num > 0
            ...
            h=[text(0.8,0.99,al,'interpreter','none'), text(0.8,0.96,sprintf('Figure %d',num),'interpreter','none')];
        else
            h=text(0.7,0.99,al,'interpreter','none');
        end
    end
    ...
    if centerTitle  > 0
        if num > 0
            % h=[text(0.7,0.93,al), text(0.7,0.91,sprintf('Figure %d',num))];
            h=[text(0.8,0.99,al,'interpreter','none'), text(0.8,0.97,sprintf('Figure %d',num),'interpreter','none')];
        else
            h=text(0.3,0.99,al,'interpreter','none');
        end
    end
